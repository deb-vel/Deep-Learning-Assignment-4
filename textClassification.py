...
index_sents = [[token2index[token] for token in sent] + [0 for _ in range(max_len - len(sent))] for sent in
               token_sents]  # Add zeros to the end of each sentence so that all sentences are equal to the maximum length (can be some other index instead of zero).
...
index_prefixes = [[token2index[token] for token in prefix] + [0 for _ in range(max_prefix_len - len(prefix))] for prefix
                  in token_prefixes]
    ...
    def __init__(self, vocab_size):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('hidden1'):
                init_state1 = tf.get_variable('init_state1', [state_size], tf.float32,
                                              tf.random_normal_initializer(stddev=init_stddev))
                batch_init1 = tf.tile(tf.reshape(init_state1, [1, state_size]), [batch_size, 1])

                cell = tf.contrib.rnn.BasicRNNCell(2, tf.tanh)
                (self.outputs1, _) = tf.nn.dynamic_rnn(cell, embedded, sequence_length=self.sent_lens,
                                                       initial_state=batch_init1)  # Pass the RNN outputs to the next layer instead of the RNN state.
                [W, b] = cell.weights
            ...
            with tf.variable_scope('hidden2'):
                init_state2 = tf.get_variable('init_state2', [state_size], tf.float32,
                                              tf.random_normal_initializer(stddev=init_stddev))
                batch_init2 = tf.tile(tf.reshape(init_state1, [1, state_size]), [batch_size, 1])

                cell = tf.contrib.rnn.BasicRNNCell(2, tf.tanh)
                (_, self.states2) = tf.nn.dynamic_rnn(cell, self.outputs1, sequence_length=self.sent_lens,
                                                      initial_state=batch_init2)  # Go through the previous RNN's outputs instead of the input words.
                [W, b] = cell.weights
                self.rnn_initialisers.extend([
                    tf.assign(W, tf.random_normal([state_size + embed_size, state_size], stddev=init_stddev)),
                    tf.assign(b, tf.zeros([state_size]))
